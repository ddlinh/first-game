extends Node
## Texture catalogue. Every sprite in the game is drawn from code at startup — no
## image files ship with the project. Fetch with Assets.tex("key").
##
## The drawing engine lives in `art.gd` (class Art); the catalogue itself is split
## by subject so each area can be authored in depth:
...
##
## Textures are baked at 2x on-screen size (Palette.PX = 0.5) with a linear filter,
## so the art stays smooth when the camera zooms in.
##
## `_bake_legacy()` below is the previous flat top-down catalogue. It runs FIRST and
## the modules overwrite it key by key, so the game is never missing a texture while
## the 2.5D art lands. It is deleted once the modules cover everything.
...
func _ready() -> void:
	...
	_bake_all()
	print("Assets: baked %d textures in %d ms" % [_tex.size(), Time.get_ticks_msec() - t0])
...
## Every registered key. Used by the contact-sheet QA tool so a newly baked
## texture cannot silently go unreviewed.
func keys() -> Array:
	return _tex.keys()
