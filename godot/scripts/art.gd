...
static func blend(im: Image, x: int, y: int, src: Color, a: float) -> void:
	...
	im.set_pixel(x, y, Color(r, g, b, oa))
...
static func paint(im: Image, prims: Array, fill: Color, outline: Color, ow: float,
	...
	var x1 := mini(im.get_width() - 1, int(ceil(bb.end.x)))
	var y1 := mini(im.get_height() - 1, int(ceil(bb.end.y)))
...
static func radial(im: Image, cx: float, cy: float, rx: float, ry: float, col: Color,
	...
	var x1 := mini(im.get_width() - 1, int(ceil(cx + rx)) + 1)
	var y1 := mini(im.get_height() - 1, int(ceil(cy + ry)) + 1)
	for y in range(y0, y1 + 1):
		for x in range(x0, x1 + 1):
