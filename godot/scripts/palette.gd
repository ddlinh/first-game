...
# --- Character scale ("Hades" framing) ---
# Actors are drawn SMALLER than props and buildings. Without this, a 156 px-tall
# hero billboard and a 156 px-tall cabin come out the same size on screen and the
# cat ends up as big as the house.
#
# At ACTOR_PX a character stands a bit over one cell tall (~53 px against a 48 px
# cell) while a cabin is ~1.6 cells, which is the proportion Hades uses: the cast
# is comfortably smaller than the architecture, and the camera (see Main's zoom)
# shows roughly 14 cells across, so there is always visible floor around a fight.
const ACTOR_PX := PX * 0.68     # 0.34
...
# Husk body tones. Kept well above true black on purpose: the dungeon multiplies
# an ambient wash over everything, and a husk painted at its intended on-screen
# darkness becomes an invisible hole in the floor once that lands.
const SHADOW   := Color("5d5080")
const SHADOW_D := Color("40355c")
...
# These are multiplied over the whole layer, and the village also runs the GWI
# grade (which adds its own vignette) on top. Tuned by eye against both: any
# darker and a cold village is unreadable rather than moody.
const AMBIENT_VILLAGE := Color("a99cb8")   # dusk; bonfire + windows warm it back
const AMBIENT_DUNGEON := Color("7f7a9c")   # gloom; braziers carve pools out of it
